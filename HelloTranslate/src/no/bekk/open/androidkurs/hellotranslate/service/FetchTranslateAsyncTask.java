...
public class FetchTranslateAsyncTask extends AsyncTask<String, String, String> {
	...
	private String[] languages = { "ar", "bg", "ca", "cs", "da", "nl", "en",
			"et", "fi", "fr" };
	...
	protected String doInBackground(String... norwegianWord) {	
		...
		for(String language : languages){
			...
			publishProgress(translatedText);
		}
		}catch(Exception ex){
			ex.printStackTrace();
			cancel(true); 
	 ...
	 * @return
	 */
	private String getTranslationForLanguage(String norwegianWord, String language) {
		String url = String.format("http://api.microsofttranslator.com/v2/Http.svc/Translate?appId=%s&text=%s&from=no&to=%s",
				MICROSOFT_API_KEY, 
				norwegianWord, 
				language);	
		...
		try {
			...
			if(response.getStatusLine().getStatusCode() != 200){
				throw new RuntimeException("Error translating the text "+norwegianWord+" to language "+language);
			}
